import {Group, Material, MathUtils, Mesh, MeshStandardMaterial, MirroredRepeatWrapping, Object3D, PlaneGeometry, ShaderMaterial, TextureLoader, Vector3} from "three";
import {scene, sceneConfiguration, water} from "../main";
import {GLTFLoader} from "three/examples/jsm/loaders/GLTFLoader";
// import {setProgress} from "./ui";

export const environmentRocks = new Array<Object3D>();
...
export let shieldModel_transparent: Object3D;
...
export const objectsInit = async () => {
    ...
    shieldModel_transparent = shieldModel.clone();
    shieldModel_transparent.traverse((child) => {
        if (child instanceof Mesh) {
            const originalMaterial = child.material as Material;
            if (!originalMaterial || !('transparent' in originalMaterial) || (originalMaterial.name === "Material.001")) {
                if (child.parent) {
...
export const objectsMotionUpdate = () => {
    environmentRocks.forEach(rock => {
        rock.position.z += sceneConfiguration.speed;
    });

    // look for the furthest rock
    if (environmentRocks[0].position.z > -1300)
        addBackgroundRock(sceneConfiguration.backgroundRockCount++, true);

    challengeRows.forEach(row => {
        row.rowParent.position.z += sceneConfiguration.speed;
    });

    // look for the furthest row
    if (challengeRows[0].rowParent.position.z > -1300) {
        addChallengeRow(sceneConfiguration.challengeRowCount++, true);
    }
    
    if (sceneConfiguration.starterBayInScene) {
        starterBay.position.z += sceneConfiguration.speed;
        
        if (starterBay.position.z > 200) {
            scene.remove(starterBay);
            sceneConfiguration.starterBayInScene = false;
        }
    }
}
...
export function updateWaterMaterial() {
    ...
    if (sceneConfiguration.rocketMoving) {
        ...
        if ((water.material as ShaderMaterial).uniforms['speed'].value > 0.0)
            (water.material as ShaderMaterial).uniforms['speed'].value -= 0.001;
        // else 
        //     (water.material as ShaderMaterial).uniforms['speed'].value = 0.0;
    }
}
export const addBackgroundRock = (count: number, horizonSpawn: boolean = false) => {
    ...
    scene.add(thisRock);
    // Add the rock to the beginning of the environmentRocks array to keep track of them (so we can clean up later)
    environmentRocks.unshift(thisRock);// add to beginning of array
}
