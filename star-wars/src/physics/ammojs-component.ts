...
export class AmmoJSRigidBody {
  body_: null | Ammo.btRigidBody = null;
  info_: null | Ammo.btRigidBodyConstructionInfo = null;
  shape_: null | Ammo.btCollisionShape = null;
  inertia_: null | Ammo.btVector3 = null;
  motionState_: null | Ammo.btDefaultMotionState = null;
  transform_: null | Ammo.btTransform = null;
  mesh_: null | Ammo.btTriangleMesh = null;
  userData_: null | Ammo.btVector3 = null;
  ...
  InitBox(pos: THREE.Vector3, quat:THREE.Quaternion, size: THREE.Vector3, userData) {
  ...
  InitMesh(src: THREE.Object3D, pos: THREE.Vector3, quat:THREE.Quaternion, userData) {
...
export class AmmoJSController extends Component {
  collisionConfiguration_: Ammo.btCollisionConfiguration;
  dispatcher_: Ammo.btCollisionDispatcher;
  broadphase_: Ammo.btDbvtBroadphase;
  solver_: Ammo.btSequentialImpulseConstraintSolver;
  physicsWorld_: Ammo.btDiscreteDynamicsWorld;
  tmpRayOrigin_: Ammo.btVector3;
  tmpRayDst_: Ammo.btVector3;
  rayCallback_: Ammo.ClosestRayResultCallback;
  ...
  RayTest(start: THREE.Vector3, end: THREE.Vector3) {
    ...
    const hits: {name: string, position: THREE.Vector3}[] = [];
  ...
  RemoveRigidBody(body: AmmoJSRigidBody) {
    this.physicsWorld_.removeRigidBody(body.body_);
    body.Destroy();
  }

  CreateBox(pos: THREE.Vector3, quat:THREE.Quaternion, size: THREE.Vector3, userData) {
  ...
  CreateMesh(src: THREE.Object3D, pos: THREE.Vector3, quat:THREE.Quaternion, userData) {
  ...
  StepSimulation(timeElapsedS: number) {
